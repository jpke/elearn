...
let Enrollable = require("./models/Enrollable");
let UserElearn = require("./models/UserElearn");
let Quiz = require("./models/Quiz").Quiz;
let SubmittedItem = require("./models/SubmittedItem");
...
let PUBLIC_KEY_ID = process.env.BOX_PUBLIC_KEY_ID;
// var PRIVATE_KEY = process.env.BOX_PRIVATE_KEY;
let PRIVATE_KEY_PASSPHRASE = process.env.BOX_PRIVATE_KEY_PASSPHRASE;
let APP_ID = process.env.BOX_APP_ID;
let ENTERPRISE_ID = process.env.ENTERPRISE_ID;

let PRIVATE_KEY = fs.readFileSync("private_key.pem", "utf-8");
let sdk = new BoxSDK({
  clientID: CLIENT_ID,
...
let box = sdk.getAppAuthClient("enterprise", ENTERPRISE_ID);
...
router.post("/users", jsonParser, function(req, res) {
  ...
  let enrolledCoursesUser = [],
    enrolledCoursesAdmin = [];
  UserElearn.findOne({ email: email })
    .exec()
    .then(function(user) {
      if (user != null) {
        throw "emailInUse";
          ...
          user
            ...
            .then(function(user) {
              ...
              console.log(
                "enrolledCoursesAdmin: ",
                enrolledCoursesAdmin,
                enrolledCoursesAdmin == null,
                enrolledCoursesAdmin == []
              );
              if (enrolledCoursesAdmin.length) {
                return Promise.all(
                  enrolledCoursesAdmin.map(function(course_id) {
            ...
            .then(function(user) {
              let courses = user.courses.map(course => {
...
router.put(
  ...
  function(req, res) {
    ...
    UserElearn.findOne({ _id: req.body.userId })
      .then(function(user) {
        if (!user) return res.status(400).json({ message: "User not found" });
        user.validatePassword(oldPassword, function(err, isValid) {
          if (err || !isValid) {
          ...
          return bcrypt.genSalt(10, function(err, salt) {
            ...
            return bcrypt.hash(newPassword, salt, function(err, hash) {
              if (err) {
                throw err;
              }
              user.password = hash;
...
router.put(
  ...
  function(req, res) {
    ...
    let courseInToken = req.user.courses.find(function(courseInToken) {
      if (courseInToken._id === course_id) return true;
    });
    if (!courseInToken.admin)
      return res
        .status(401)
        .json({ message: "only course admin can delete users from course" });
    if (req.body.email === ADMIN_EMAIL)
      return res
        .status(401)
        .json({ message: "Unable to delete site admin from course" });
    UserElearn.findOneAndUpdate(
...
router.get(
  ...
  function(req, res) {
    ...
    if (!courseInToken.admin)
      return res
        .status(401)
        .json({ message: "only course admin can see enrolled users" });
    Enrollable.find({ course_id: course_id })
      .exec()
      .then(function(enrollable) {
        return UserElearn.find({ courses: course_id })
...
router.post(
  ...
  function(req, res) {
    ...
    if (!courseInToken.admin)
      return res.status(401).json({ message: "only admin can alter course" });
    let enrolled = new Enrollable();
...
router.delete(
  ...
  function(req, res) {
    ...
    if (!courseInToken.admin)
      return res.status(401).json({ message: "only admin can alter course" });
    Enrollable.findOne({ email: req.body.email, course_id: course_id })
      .remove()
      .exec()
...
router.get(
  ...
  function(req, res) {
    Quiz.find({ _id: req.params.quizId })
      .exec()
      .then(function(quiz) {
        return SubmittedItem.find({
          user: req.params.userId,
          of: req.params.quizId
        })
          .exec()
...
router.put(
  ...
  function(req, res) {
    ...
    let course = req.user.courses.find(function(course) {
    ...
    if (quiz.courses[0] === "undefined")
      quiz.courses[0] = { id: req.body.courseID };
...
router.delete(
  ...
  passport.authenticate("bearer", { session: false }),
  function(req, res) {
    let courseInToken = req.user.courses.find(function(course) {
      if (course._id === req.params.courseId) return true;
    });
    if (!courseInToken.admin)
      return res.status(401).json({ message: "only admin can alter course" });
    let course = Course.findOneAndUpdate(
...
router.post(
  ...
  function(req, res) {
    ...
    let submission = new SubmittedItem();
    ...
    submission
      ...
      .then(function(submission) {
        return Quiz.findOne({ _id: submission.of }).exec().then(function(quiz) {
      ...
      .then(function(state) {
        if (state[1]) {
          return UserElearn.findOne({ _id: req.body.user_Id })
            .exec()
            .then(function(user) {
