...
const CourseListCreator = (items, selectFunction, passed, token, url) => {
  return items.map((item, index) => {
    ...
    if(passed) {
      ...
      if(qSum.length === item.quizzes.length) {
        return (
          <li key={index} onClick={() => selectFunction(item)}>{itemName}
            <a href={url + "users/certificate/".concat(item.name, "/", token)} target="_blank">
...
export default CourseListCreator;
